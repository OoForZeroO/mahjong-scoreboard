...
-- 注意：在PostgreSQL中，创建数据库需要在连接到postgres数据库时执行
-- 此脚本假设您已经连接到正确的数据库
-- 如果需要创建数据库，请在psql命令行执行：CREATE DATABASE mahjongdb;
...
-- 插入示例数据（可选）
-- 注意：这里使用了明文密码，实际环境中应该使用加密存储
INSERT INTO users (username, phone, email, password, role, status, avatar, create_time, update_time)
VALUES 
('张三', '13800138001', 'zhangsan@example.com', '123456', 'admin', 'active', 'https://example.com/avatar1.jpg', EXTRACT(EPOCH FROM NOW())::bigint, EXTRACT(EPOCH FROM NOW())::bigint),
('李四', '13800138002', 'lisi@example.com', '123456', 'user', 'active', 'https://example.com/avatar2.jpg', EXTRACT(EPOCH FROM NOW())::bigint, EXTRACT(EPOCH FROM NOW())::bigint),
('王五', '13800138003', 'wangwu@example.com', '123456', 'user', 'active', 'https://example.com/avatar3.jpg', EXTRACT(EPOCH FROM NOW())::bigint, EXTRACT(EPOCH FROM NOW())::bigint)
ON CONFLICT (phone) DO NOTHING;

-- 插入示例棋牌室数据
INSERT INTO rooms (name, logo, create_time, update_time)
VALUES 
('快乐棋牌室', 'https://example.com/room1.jpg', EXTRACT(EPOCH FROM NOW())::bigint, EXTRACT(EPOCH FROM NOW())::bigint),
('休闲麻将馆', 'https://example.com/room2.jpg', EXTRACT(EPOCH FROM NOW())::bigint, EXTRACT(EPOCH FROM NOW())::bigint),
('友谊棋牌室', 'https://example.com/room3.jpg', EXTRACT(EPOCH FROM NOW())::bigint, EXTRACT(EPOCH FROM NOW())::bigint)
ON CONFLICT (name) DO NOTHING;

-- 插入示例对局数据
INSERT INTO matches (room_id, room_name, start_time, status, total_rounds, current_round, create_time, update_time)
VALUES 
(1, '快乐棋牌室', EXTRACT(EPOCH FROM NOW())::bigint, '进行中', 0, 0, EXTRACT(EPOCH FROM NOW())::bigint, EXTRACT(EPOCH FROM NOW())::bigint)
ON CONFLICT DO NOTHING;

-- 为示例对局添加参与者
INSERT INTO match_participants (match_id, user_id, nickname, avatar, total_score, create_time, update_time)
SELECT m.match_id, u.id, u.username, u.avatar, 0, EXTRACT(EPOCH FROM NOW())::bigint, EXTRACT(EPOCH FROM NOW())::bigint
FROM matches m, users u
WHERE m.room_id = 1 AND u.id IN (1, 2, 3)
ON CONFLICT DO NOTHING;
